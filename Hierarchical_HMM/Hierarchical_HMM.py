...
class Hierarchical_HMM:
    ...
    def eta_in(self, t, i, state, output_sequence):
        if (state.parent == None):
            if (t == 1):
                ...
                answer = 0
                for j in range(len(state.substates)):
                    answer += self.alpha(self, 1, t - 1, j, state, output_sequence) * state.a[j][i]
                return answer
            else:
                print("eta_in error")
        else:
            if (t == 1):
                return self.eta_in(1, state.state_index, state.parent, output_sequence) * state.pi[i]
            elif (t > 1):
                answer1 = 0
                for t_prime in range(t - 1):
                    answer2 = 0
                    for j in range(len(state.substates)):
                        answer2 += self.alpha(t_prime, t - 1, j, state, output_sequence) * state.a[j][i]

                    answer1 += self.eta_in(t_prime, state.state_index, state.parent, output_sequence) * answer2
                answer += self.eta_in(t, state.state_index, state.parent, output_sequence) * state.pi[i]
            else:
                print("eta_in error")
    ...
    def eta_out(self, t, i, state, output_sequence):
        if (t < len(output_sequence)):
            if (state.parent == None):
                answer = 0
                for j in range(len(state.substates)):
                    answer += state.a[i][j] * self.beta(t + 1, len(output_sequence), j, state, output_sequence)
                return answer
            else:
                answer = 0
                for k in range(t+1, len(output_sequence)):
                    answer2 = 0
                    for j in range(len(state.substates)):
                        answer2 += state.a[i][j] * self.beta(t+1, k, j, state.state_index, output_sequence)
                    answer2 *= self.eta_out(k, state.state_index, state.parent, output_sequence)
                    answer += answer2
                answer += state.a[i][0] * self.eta_out(len(output_sequence), state.state_index, state.parent, output_sequence)
                return answer
        else:
            return state.a[i][0] * self.eta_out(len(output_sequence), state.state_index, state.parent, output_sequence)
    ...
    def xi(self, t, i, j, state, output_sequence):
        if (state.parent == None):
            if (t < len(output_sequence)):
                ...
                return self.alpha(1, len(output_sequence), i, state, output_sequence) * state.a[i][j] / self.output_sequence_prob(output_sequence)
            ...
            if (not (j == 0)):
                ...
                return  answer1 * state.a[i][j] * answer2 / self.output_sequence_prob(output_sequence)
            else:
                answer = 0
                for s in range(t):
                    answer += self.eta_in(s, state.state_index, state.parent, output_sequence) * self.alpha(s, t, i, state, output_sequence)
                answer *= state.a[i][0] * self.eta_out(t, state.state_index, state.parent, output_sequence)
                answer /= self.output_sequence_prob(output_sequence)
                return answer
    
    def chi(self, t, i, state, output_sequence):
        if (state.parent == None):
            return state.pi[i] * self.beta(1, len(output_sequence), i, state, output_sequence) / self.output_sequence_prob(output_sequence)
        else:
            answer = 0
            ...
            answer *= self.eta_in(t, state.state_index, state.parent, output_sequence) * state.pi(i) / self.output_sequence_prob(output_sequence)
    ...
    def _update_pi(self, state, output_sequence):
        if (state.parent == None):
            ...
            for i in range(len(state.substates)):
                ...
                state.pi[i] = numerator / denominator
    
    def _update_a(self, state, output_sequence):
        for i in range(len(state.substates)):
            for j in range(len(state.substates)):
                numerator = 0
                denominator = 0
                ...
                state.a[i][j] = numerator / denominator
    ...
    def _update(self, state, output_sequence):
        if (state.isProductionState):
            ...
            self._update_pi(state, output_sequence)
            self._update_a(state, output_sequence)
            for i in range(len(state.substates)):
